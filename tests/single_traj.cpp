  ...
  //check cp
    ...
    if(critical_points_0.find(simplex_offset) != critical_points_0.end()){
        ...
        if(critical_points_dec.find(simplex_offset) != critical_points_dec.end()){
            ...
            double seed_coord[2] = {cp.x[0] + eps*cp.eig_vec[0][0], cp.x[1] + eps*cp.eig_vec[0][1]};
            printf("cp coord: %f, %f\n", cp.x[0], cp.x[1]);
            printf("seed coord: %f, %f\n", seed_coord[0], seed_coord[1]);
    ...
    std::string filename = "single_traj_" + std::string(argv[7]) + "_" + std::string(argv[8]) + ".bin";
    write_trajectory(traj_ori, filename);
    std::string filename_dec = "single_traj_dec_" + std::string(argv[7]) + "_" + std::string(argv[8]) + ".bin";
    write_trajectory(traj_dec, filename_dec);
    //write index to file
    std::string filename_index = "single_traj_index_" + std::string(argv[7]) + "_" + std::string(argv[8]) + ".bin";
    writefile(filename_index.c_str(), traj_index_ori.data(), traj_index_ori.size());
    std::string filename_index_dec = "single_traj_index_dec_" + std::string(argv[7]) + "_" + std::string(argv[8]) + ".bin";
